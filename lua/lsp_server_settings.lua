...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(event)
		...
		end, opts("LSP: Toggle document diagnostic"))
		...
		if client ~= nil then
			if client.name == "rust_analyzer" then
				...
				vim.keymap.set(
					"n",
					"<leader>btp",
					"<cmd>CargoTestPrint<CR>",
					opts("Cargo test, with no capture and 1 job, for print output")
				)
		...
		if
			client
			and functions.client_supports_method(client, vim.lsp.protocol.Methods.textDocument_inlayHint, event.buf)
		then
